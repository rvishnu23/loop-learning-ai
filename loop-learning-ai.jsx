...
async function generatePracticeQuiz({ subject, chapter, topic, difficulty, spec }) {
  ...
  const system =
    ...
    "For subjective: concise modelAnswer + 2-4 short markingPoints. Every question needs a brief explanation. Respond with " +
...
function PersonalPractice({ db, setDb, student, seedTopic }) {
  ...
  return (
    <div className="max-w-3xl space-y-5">
      ...
      {quiz && submitted && (<Card className="p-5 space-y-4"><div className="flex items-center justify-between"><div className="text-lg font-black text-stone-800">Score: {(objectiveScore() + Object.values(evaluations).reduce((s, e) => s + (e.marksAwarded || 0), 0)).toFixed(1)} / {quiz.reduce((s, q) => s + q.marks, 0)}</div><Badge tone="amber">Personal Practice — Not Official</Badge></div>{quiz.map((q, i) => { if (q.type === "subjective") { const e = evaluations[i]; return (<div key={i} className="p-3 rounded-xl bg-stone-50 text-sm"><div className="font-medium text-stone-800 mb-1">{i + 1}. {q.question}</div><div className="text-stone-500 text-xs mb-1">Your answer: {answers[i]}</div>{e ? (<><div className="font-bold text-indigo-700 text-xs mb-1">{e.marksAwarded} / {q.marks} marks</div><div className="text-xs text-teal-700">✓ {e.whatWasCorrect}</div><div className="text-xs text-rose-600">Missing: {e.whatWasMissing}</div><div className="text-xs text-stone-500">Concept: {e.misunderstoodConcept}</div><div className="text-xs text-indigo-600">Next: {e.nextPractice}</div></>) : <div className="text-xs text-stone-400">Model answer: {q.modelAnswer}</div>}</div>); } const correct = q.type === "mcq" ? answers[i] === q.correctIndex : q.type === "tf" ? answers[i] === q.correctAnswer : (answers[i] || "").trim().toLowerCase() === String(q.correctAnswer).trim().toLowerCase(); return (<div key={i} className={`p-3 rounded-xl text-sm ${correct ? "bg-teal-50" : "bg-rose-50"}`}><div className="font-medium text-stone-800 mb-1">{i + 1}. {q.question}</div><div className={correct ? "text-teal-700" : "text-rose-700"}>Your answer: {q.type === "mcq" ? q.options[answers[i]] : q.type === "tf" ? (answers[i] ? "True" : "False") : answers[i]} {correct ? "✓" : "✗"}</div>{!correct && <div className="text-stone-600">Correct answer: {q.type === "mcq" ? q.options[q.correctIndex] : q.type === "tf" ? (q.correctAnswer ? "True" : "False") : q.correctAnswer}</div>}<div className="text-stone-500 text-xs mt-1">{q.explanation}</div></div>); })}<GhostButton onClick={() => { setQuiz(null); setSubmitted(false); }} icon={RefreshCw}>Practise another topic</GhostButton></Card>)}
