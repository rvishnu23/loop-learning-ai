...
import {
  ...
  Radio, PlayCircle, StopCircle, Clock, Paperclip, Wand2, Copy, Sigma, FlaskConical,
} from "lucide-react";
import {
...
const SYMBOL_GROUPS = {
  Math: ["²", "³", "√", "∞", "π", "θ", "Δ", "∑", "∫", "≈", "≠", "≤", "≥", "±", "×", "÷", "→", "↔", "∈", "∝"],
  Chemistry: ["→", "⇌", "↑", "↓", "Δ", "°C", "°", "H₂O", "CO₂", "O₂", "Na⁺", "Cl⁻", "Ca²⁺", "NH₄⁺", "SO₄²⁻", "NO₃⁻", "⇢", "(s)", "(l)", "(g)", "(aq)"],
};
function SymbolInput({ value, onChange, multiline = false, className = "", ...props }) {
  ...
  const inputClass = `w-full px-3 py-2 pr-11 rounded-lg border border-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 ${className}`;
  return (
    <div className="relative flex-1">
      {multiline ? <textarea ref={inputRef} {...props} value={value} onChange={onChange} className={inputClass} /> : <input ref={inputRef} {...props} value={value} onChange={onChange} className={inputClass} />}
      <button type="button" title="Insert a mathematical or chemistry symbol" aria-label="Insert a mathematical or chemistry symbol" onMouseDown={(e) => e.preventDefault()} onClick={() => setOpen((current) => !current)} className="absolute right-2 top-2 p-1 rounded text-stone-500 hover:bg-indigo-50 hover:text-indigo-700"><Sigma size={17} /></button>
      {open && <div className="absolute right-0 bottom-full mb-2 z-20 w-72 rounded-xl border border-stone-200 bg-white p-2 shadow-lg">
        <div className="flex gap-1 mb-2">
          {Object.keys(SYMBOL_GROUPS).map((name) => <button key={name} type="button" onClick={() => setGroup(name)} className={`flex-1 inline-flex items-center justify-center gap-1 rounded-lg px-2 py-1.5 text-xs font-semibold ${group === name ? "bg-indigo-700 text-white" : "text-stone-600 hover:bg-stone-100"}`}>{name === "Math" ? <Sigma size={13} /> : <FlaskConical size={13} />}{name}</button>)}
        </div>
        <div className="grid grid-cols-7 gap-1">{SYMBOL_GROUPS[group].map((symbol, index) => <button key={`${symbol}-${index}`} type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => insertSymbol(symbol)} className="min-h-8 rounded-md border border-stone-100 text-sm text-stone-700 hover:border-indigo-300 hover:bg-indigo-50">{symbol}</button>)}</div>
        <div className="mt-2 text-[10px] text-stone-400">Symbols are inserted where your cursor is placed.</div>
      </div>}
    </div>
  );
}
      ...
      <div className="space-y-3">
        {questions.map((q, i) => (
          <Card key={q.id || i} className="p-4 space-y-2">
            ...
            <SymbolInput multiline rows={2} value={q.question} onChange={(e) => updateQ(i, { question: e.target.value })} />
  ...
  return (
    <div className="max-w-2xl space-y-4">
      ...
      <Card className="p-5 space-y-3">
        ...
        {q.type === "fill" && <SymbolInput placeholder="Your answer" value={answers[qi] || ""} onChange={(e) => setAnswer(qi, e.target.value)} />}
        {q.type === "subjective" && <SymbolInput multiline rows={4} placeholder="Write your answer…" value={answers[qi] || ""} onChange={(e) => setAnswer(qi, e.target.value)} />}
      </Card>
      <div className="flex items-center justify-between">
...
function PersonalAITutor({ db, setDb, student }) {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      ...
      <div className="flex gap-2 items-center"><FileUploadChip onFile={attachToMessage} busy={uploading || loading} /><SymbolInput placeholder="Type your question…" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} /><PrimaryButton icon={Send} onClick={() => send()} disabled={loading}>Ask</PrimaryButton></div>
    </div>
  );
}
